...
class ContainerConfAudit(ConfAudit):
    ...
    def getConfUpdate(self, conf_obj: AuditUpdate, parent=None) -> QWidget:
        ...
        if conf_obj.field == "tag":
            conf_audit.name.setText(tr("Image Tag changed from %s to %s") % (conf_obj.value_from, conf_obj.value_to))
        elif conf_obj.field == "memory_limit":
            conf_audit.name.setText(tr("Amount of Memory allowed is limited to %sMB") % conf_obj.value_to)
        elif conf_obj.field == "cpu_limit":
            conf_audit.name.setText(tr("Number of CPU allowed is limited to %s cpus") % conf_obj.value_to)
        elif conf_obj.field == "entrypoint":
            conf_audit.name.setText(tr("Entrypoint changed to %s") % conf_obj.value_to)
