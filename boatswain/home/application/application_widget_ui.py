...
from boatswain.common.services.system_service import applyRatio, applyFontRatio
...
class AppWidgetUi(QWidget):
    ...
    def __init__(self, parent, container: Container, handler) -> None:
        ...
        self.vertical_layout.setContentsMargins(0, 0, applyRatio(2), 0)
        ...
        self.horizontal_layout.setContentsMargins(applyRatio(20), applyRatio(1), applyRatio(10), applyRatio(3))

        img_name = container.image_name
        name_part = container.image_name.split('/')
        if len(name_part) > 1:
            img_name = name_part[1]
        self.pic = AppAvatar(text_utils.getSimpleName(img_name), parent=self.widget, radius=applyRatio(20))
        ...
        padding = "%dpx %dpx" % (applyRatio(1), applyRatio(10))
